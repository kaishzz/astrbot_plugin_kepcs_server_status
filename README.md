...
- 通过 `kepcs_status` 指令查询服务端状态
- 按 `Training map`、`Play map` 分组展示
- 显示地图、人数和加入链接
- 自动隐藏非空闲且仍可用的繁忙服务器
- 统计总在线人数
...
- `api_key` 和 `bearer_token` 至少填写一个；如果只填写 `bearer_token`，插件会仅发送 `Authorization` 请求头。
